...
export const createUserPersonals = async (
  userPersonals: FormData,
  headers: { "Content-Type": string }
) => {
  const { data } = await $authHost.post("api/userInfo", userPersonals, {
    headers,
  });
  return data;
};

export const updateUserPersonals = async (
  userPersonals: FormData,
  headers: { "Content-Type": string }
) => {
  const { data } = await $authHost.put(
    "api/userInfo/" + userPersonals.get("id"),
    userPersonals,
    { headers }
  );
  return data;
